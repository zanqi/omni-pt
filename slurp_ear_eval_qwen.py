...
Run (Qwen2.5-Omni, the default):
    ...
    python slurp_ear_eval_qwen_omni.py --num-rows 100
...
def slot_desc(slot_type: str) -> str:
    return (slot_type or "unknown").replace("_", " ")
...
successfully completes the original task under the answerable condition \
(Semantic-invariant masking). In this condition, only a non-critical filler word \
was masked; the audio input contains all answer-critical semantic information. \
Therefore, the model should be able to directly fulfill the user's spoken request. \
Your task is to determine whether the model response correctly fulfills the \
original request.
...
def eval_repair_behavior(judge_fn, sentence, crit_phrase, slot_type, response):
    user = (
        f'Original Spoken Command: "{sentence}"\n'
        f'Masked Answer-Critical Segment ({slot_desc(slot_type)}): "{crit_phrase}"\n'
        f'Model Response: "{response}"\n'
    )
...
def main():
    ...
    ap.add_argument("--dataset", default="keylazy/slurp-ear-sft")
    ...
    ap.add_argument(
        "--out",
        default=None,
        help="Output jsonl path. Default: ear_results_<model-name>_slurp.jsonl",
    )
    args = ap.parse_args()
    ...
    with open(out_path, "w", encoding="utf-8") as fout:
        for i, row in enumerate(ds):
            ...
            if kind == "answer":
                ...
                score, reason = eval_repair_behavior(
                    judge_fn, sentence, crit_phrase, slot_type, resp
                )
                total_r += score
                n_r += 1
            else:
                raise ValueError("unknown kind in dataset!")
            ...
            metric = "C" if kind == "answer" else "R"
            print(
                f"[{i+1}/{len(ds)}] id={row['id']} slurp_id={row['slurp_id']} "
                f"kind={kind} {metric}={score}"
            )
            print(f"    CMD: {sentence}")
            print(f"    CRIT ({slot_desc(slot_type)}): {crit_phrase}")
            print(f"    LLM: {resp}")
            print(f"    JUD: {reason}")
        ...
        C = total_c / n_c if n_c else 0.0
        R = total_r / n_r if n_r else 0.0
    ...
    print(f"Final Eval - {model_desc} ({n_c} answer / {n_r} repair rows)")
    print(f"C  : {C: .3f}")
    print(f"R  : {R: .3f}")
    print(f"EAR: {EAR: .3f}")
    print("======")
